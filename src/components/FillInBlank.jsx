...
function fuzzyMatch(input, answer) {
    const a = input.trim().toLowerCase().replace(/[^a-z0-9.]/g, '')
    const b = answer.trim().toLowerCase().replace(/[^a-z0-9.]/g, '')
    if (a === b) return true
...
export default function FillInBlank() {
    ...
    const startQuiz = () => {
        const qs = getQuizQuestions('fill-blank', difficulty, selectedCats, 10)
    ...
    const handleNext = () => {
        if (current + 1 >= questions.length) {
            ...
            recordQuizScore('fill-blank', score, questions.length, difficulty)
            setFinished(true)
        } else {
    ...
    if (!started) return (
        <div className="animate-in">
            <div className="page-header"><h2>✏️ Fill in the Blank</h2><p>Type the missing word or number</p></div>
            <div className="mb-6">
            ...
            <button className="btn btn-primary btn-lg" onClick={startQuiz}>Start →</button>
        </div>
    )
    ...
    if (finished) return (
        <div className="animate-in">
            <div className="quiz-score-card card-glass">
                ...
                <h2 className="mb-2">{pct >= 80 ? '🎉 Great Job!' : pct >= 50 ? '👍 Good Effort!' : '💪 Keep Studying!'}</h2>
    ...
    return (
        <div className="animate-in">
            <div className="quiz-container">
                ...
                {showHint && !answered && (
                    <div style={{ color: 'var(--color-warning)', fontSize: 'var(--font-size-sm)', marginBottom: 'var(--space-2)' }}>
                        💡 Hint: Starts with "{q.answer[0]}", {q.answer.length} letters
                    </div>
                )}
                <form onSubmit={handleSubmit} className="flex gap-4 items-center justify-center mt-6">
                ...
                {!answered && !showHint && <div className="text-center mt-4"><button className="btn btn-sm btn-secondary" onClick={() => setShowHint(true)}>Need a hint?</button></div>}
                {answered && (
                    <>
